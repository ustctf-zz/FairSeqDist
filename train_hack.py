...
def init_epoch_iter(task, trainer, args, dummy_batch, max_positions):
...
def main(args, init_distributed=False):
    ...
    while lr > args.min_lr and global_epoch < max_epoch and trainer.get_num_updates() < max_update:
        # train for one epoch
        epoch_itr = init_epoch_iter(task=task, trainer=trainer, max_positions= max_positions, dummy_batch= dummy_batch, args=args)

        print('rank {}, start epoch {}, len {}, global_epoch {}'.format(args.distributed_rank, epoch_itr.epoch, len(epoch_itr), global_epoch))
        train(args, trainer, task, epoch_itr)

        if global_epoch % args.validate_interval == 0:
            valid_losses = validate(args, trainer, task, epoch_itr, valid_subsets)

        # only use first validation loss to update the learning rate
        lr = trainer.lr_step(global_epoch, valid_losses[0])

        # save checkpoint
        if global_epoch % args.save_interval == 0:
