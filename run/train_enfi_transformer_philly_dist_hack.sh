...
FullSaveDir=${SaveDir}/${Dataset}_${Arch}_dp${dropout}_seed${seed}_maxtok${MaxTokens}_lr${LR}_SI${${SaveInterval}}_${Extra}_hack
LogFilename=${Dataset}-${Arch}-dp${dropout}-seed${seed}-maxtok${MaxTokens}-lr${LR}-SI${${SaveInterval}}-${Extra}_hack
DistFilename=${DistDir}/${Dataset}-${Arch}-dp${dropout}-seed${seed}-maxtok${MaxTokens}-lr${LR}-SI${${SaveInterval}}-${Extra}_hack
...
python -m torch.distributed.launch --nproc_per_node=${NProcPerNode} \
    ...
    --log-interval 10 \
    --save-interval ${SaveInterval} --save-interval-updates 10000 --keep-interval-updates 0 \
