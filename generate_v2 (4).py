...
def decode_from_file(models, task, args, use_cuda, source_filename=None,
    ...
    dataset = data.LanguagePairDataset(
        src_tokens, src_sizes, src_dict,
        tgt_tokens, tgt_sizes, tgt_dict,
        left_pad_source=args.left_pad_source, left_pad_target=args.left_pad_target,
        max_source_positions=args.max_source_positions, max_target_positions=args.max_target_positions,
        shuffle=False, r2l=args.r2l)
    itr = task.get_batch_iterator(
        dataset=dataset,
        max_tokens=args.max_tokens,
        max_sentences=args.max_sentences,
